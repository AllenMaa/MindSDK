...
+ 初始化 FLEXCAN ，初始化 FLEXCAN FD，配置传输会使用到的最大Mb数目，配置时钟源和波特率，配置总线唤醒源，配置工作模式为Normal模式，配置不自接收，配置时钟和同步设置。
...
app_flexcan_tx() sending: 0 1 2 3 4 5 6 7 9 A B C D E F
app_flexcan_tx() done.

app_flexcan_tx() sending: 0 2 4 6 8 A C E 10 12 14 16 18 1A 1C 1E
app_flexcan_tx() done.