...
uint8_t app_flexcan_tx_buf[APP_FLEXCAN_XFER_BUF_LEN] = {0u, 1u, 2u, 3u, 4u, 5u, 6u, 7u}; /* The flexcan tx buffer for tx mb frame preparation. */
uint8_t app_flexcan_rx_buf[APP_FLEXCAN_XFER_BUF_LEN]; /* The flexcan rx buffer for rx mb frame reception. */

/*
 * Declerations.
 */
void app_flexcan_init(void);            /* Setup flexcan. */
void app_flexcan_ack(uint8_t *tx_buf);  /* Transport frame. */
...
int main(void)
{
    ...
    app_flexcan_init();
    /* Defaultedly only check the id without frame format and frame type. */
    printf("waiting for any frames with id 0x%x.\r\n", (unsigned)APP_FLEXCAN_XFER_ID);
    ...
    while (1)
    {
        ...
        printf("app_flexcan_req() at timestamp %u with id %u.\r\n", (unsigned)app_flexcan_rx_mb.TIMESTAMP, (unsigned)app_flexcan_rx_mb.ID);

        printf("app_flexcan_ack() sending: ");
        /* Send a message through flexcan. */
        for (uint8_t i = 0u; i < APP_FLEXCAN_XFER_BUF_LEN; i++)
        {
           app_flexcan_tx_buf[i] = (app_flexcan_tx_buf[i] + 1) % 256u;
           printf("%u ", (unsigned)app_flexcan_tx_buf[i]);
        }
        printf("\r\n");
        app_flexcan_ack(app_flexcan_tx_buf);
...
void app_flexcan_init(void)
{
    ...
    flexcan_mb_conf.MbType = FLEXCAN_MbType_Remote; /* Only receive standard data frame. */
...
void app_flexcan_ack(uint8_t * tx_buf)
{
    FLEXCAN_Mb_Type mb;
