...
void CLOCK_EnableLSI40kHz(void);
void LPTIM_CLOCK_Set(uint8_t clokc_source);

void BOARD_InitBootClocks(void)
{
    CLOCK_ResetToDefault();
    CLOCK_BootToHSE72MHz();
    CLOCK_EnableLSI40kHz();
    
    /* LPTIM clock set. */
    LPTIM_CLOCK_Set(CLOCK_LPTIM_LSI);
    ...
    /* LPTIM. */
    RCC_EnableAPB2Periphs(RCC_APB2_PERIPH_LPTIM, true);
...
{
    /* Enable LSI */
    RCC->CSR |= (RCC_CSR_LSION_MASK | RCC_CSR_LSIOE_MASK); /* Make sure the LSI is enabled. */
    while( RCC_CSR_LSIRDY_MASK != (RCC->CSR & RCC_CSR_LSIRDY_MASK) )
    {
    }
}
...
void LPTIM_CLOCK_Set(uint8_t clokc_source)
{
    RCC->CFGR2 = ( (RCC->CFGR2 &~ RCC_CFGR2_LPTIMCLKSEL_MASK)
                 | RCC_CFGR2_LPTIMCLKSEL(clokc_source)
                 );
}
