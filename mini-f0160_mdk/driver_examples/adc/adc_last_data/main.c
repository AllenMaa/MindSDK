...
int main(void)
{
    uint32_t ldata; /* the previous conversion result. */
    uint32_t flags;
    uint32_t adc_channel; /* keep the actual hardware conversion channel number. */

    BOARD_Init();
    printf("adc_last_data example.\r\n");
    app_adc_init();
    printf("press any key to start the conversion.\r\n");

    while (1)
    {
        /* press any key into the terminal to trigger the conversion. */
        getchar();

        /* start the one channel conversion. */
        printf("app_adc_run_conv() start...\r\n");
        printf("adc_val= %u\r\n", (unsigned)app_adc_run_conv());
        ldata = ADC_GetLastConvResult(BOARD_ADC_PORT, &adc_channel, &flags);
        printf("adc_last_val= %u\r\n", (unsigned)ldata);
