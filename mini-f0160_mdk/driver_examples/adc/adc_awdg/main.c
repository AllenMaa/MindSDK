 ...
 * Variables.
 */
uint32_t app_adc_compare_channel = 1u;
uint32_t conv_val;
...
void app_adc_init(void)
{
    ...
    ADC_SetChnSampleTime(BOARD_ADC_PORT, app_adc_compare_channel, ADC_SampleTime_Alt7);
    ...
    ADC_EnableInterrupts(BOARD_ADC_PORT, ADC_INT_CONV_COMPARE_DONE, true);
...
void BOARD_ADC_IRQHandler(void)
{
    ...
    if ( 0u != (flags & ADC_STATUS_CONV_COMPARE_DONE) )
    {
