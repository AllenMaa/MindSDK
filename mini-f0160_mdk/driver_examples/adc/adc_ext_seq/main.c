...
int main(void)
{
    ...
    while (1)
    {
        ...
        printf("seq conversion start...\r\n");

        ADC_DoSwTrigger(BOARD_ADC_PORT, true);
        ADC_DoExtSeqSlotSwTrigger(BOARD_ADC_PORT, true); /* start inject channel conversion. */
        ...
        printf("seq conversion done.\r\n\r\n");
        uint32_t flags;
        uint32_t adc_channel; /* keep the actual hardware conversion channel number. */
        uint32_t data;
        while( 0u == (ADC_GetStatus(BOARD_ADC_PORT) & ADC_STATUS_JCONV_SEQ_DONE) )
        {}

        ADC_ClearStatus(BOARD_ADC_PORT, ADC_STATUS_JCONV_SEQ_DONE);

        data = ADC_GetExtConvResult(BOARD_ADC_PORT, &adc_channel, &flags);
        printf("ext_value=%u\r\n", (unsigned)data);
        printf("ext_seq conversion done.\r\n\r\n");
    }
...
void app_adc_init(void)
{
    ...
    adc_init.ConvMode = ADC_ConvMode_SeqOneTime; /* run the conversion by sequence. */
    ...
    for (uint32_t i = 0u; i < APP_ADC_SEQ_LEN; i++)
    {
       ADC_EnableSeqSlot(BOARD_ADC_PORT, i, app_adc_seq_channels[i]); /* place the channel in the slot. */
       ADC_SetChnSampleTime(BOARD_ADC_PORT, app_adc_seq_channels[i], ADC_SampleTime_Alt7);
    }
    /* setup the sample time for each channel. */
    for (uint32_t i = 0u; i < APP_ADC_SEQ_LEN; i++)
    {
        ADC_EnableExtSeqSlot(BOARD_ADC_PORT, i, app_adc_extseq_channels[i]);
        ADC_SetChnSampleTime(BOARD_ADC_PORT, app_adc_extseq_channels[i], ADC_SampleTime_Alt7);
    }

    /* enable the DMA for transporting data. */
    ADC_EnableDMA(BOARD_ADC_PORT, true);
    /* enable the inject atuo conversion. */
    ADC_EnableAutoExtSeqSlot(BOARD_ADC_PORT, true);
}

/* entry to dma transfer done interrupt. */
void BOARD_DMA_ADC_IRQHandler(void)
{
    ADC_DoSwTrigger(BOARD_ADC_PORT, false);
    ADC_DoExtSeqSlotSwTrigger(BOARD_ADC_PORT, false);
