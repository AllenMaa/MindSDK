...
/* Configure wake up pin parameters. */
static PWR_WakeUpPinConf_Type WakeUpPinConf =
{
    .Pins = BOARD_POWER_STANDBYWAKEUP_PORT,
    .TrgIn = PWR_WakeUpPinTriggerIn_RisingEdge,
};
...
{
    UART_Init_Type uart_init;
    uart_init.ClockFreqHz   = ClockFreq; /* 24mhz, APB1. */
    uart_init.BaudRate      = BOARD_DEBUG_UART_BAUDRATE;
    uart_init.WordLength    = UART_WordLength_8b;
    uart_init.StopBits      = UART_StopBits_1;
    uart_init.Parity        = UART_Parity_None;
...
void BOARD_PowerModeSelect(uint8_t ch)
{
    switch (ch)
    {
        ...
        case 'e':
            ...
            POWER_EnableWakeUpPin(&WakeUpPinConf);
